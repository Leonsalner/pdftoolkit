...
pub async fn detect_smart_cards() -> Result<Vec<SmartCardInfo>, String> {
    let ctx = Context::establish(Scope::User).map_err(|e| format!("Failed to establish PCSC context: {}", e))?;
...
pub async fn sign_pdf_file_based(
    ...
    std::fs::copy(&input_path, &output_path).map_err(|e| format!("Failed to create output: {}", e))?;
