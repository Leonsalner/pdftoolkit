...
pub async fn add_text_watermark(
    ...
    for page_num in target_pages.clone() {
        let (width, height, resources_id_opt) = {
            ...
            let page_dict = doc.objects.get(&page_id).and_then(|o| o.as_dict().ok()).ok_or("Failed to get page dict")?;
            ...
            if let Ok(media_box) = page_dict.get(b"MediaBox") {
                if let Ok(arr) = media_box.as_array() {
                    if arr.len() == 4 {
                        w = arr[2].as_float().or_else(|_| arr[2].as_i64().map(|v| v as f32)).unwrap_or(612.0);
                        h = arr[3].as_float().or_else(|_| arr[3].as_i64().map(|v| v as f32)).unwrap_or(792.0);
                    }
                }
            }

            let res_id = page_dict.get(b"Resources").ok().and_then(|obj| obj.as_reference().ok());
            (w, h, res_id)
        };
        ...
        let content = Content {
            operations: vec![
                Operation::new("q", vec![]),
                Operation::new("rg", vec![Object::Real(color.0), Object::Real(color.1), Object::Real(color.2)]),
                Operation::new("BT", vec![]),
                Operation::new("Tf", vec![Object::Name(b"F1".to_vec()), Object::Real(font_size)]),
                {
                    let rad = rotation.to_radians();
                    let cos_a = rad.cos();
                    let sin_a = rad.sin();
                    Operation::new("Tm", vec![
                        Object::Real(cos_a), Object::Real(sin_a), 
                        Object::Real(-sin_a), Object::Real(cos_a),
                        Object::Real(width / 2.0), Object::Real(height / 2.0)
                    ])
                },
                Operation::new("Td", vec![Object::Real(-(text.len() as f32 * font_size * 0.3)), Object::Real(0.0)]),
                Operation::new("Tj", vec![Object::String(text.clone().into_bytes(), lopdf::StringFormat::Literal)]),
        ...
        if let Some(Object::Dictionary(page_dict)) = doc.objects.get_mut(&page_id) {
            if let Ok(contents) = page_dict.get(b"Contents") {
                if let Ok(arr) = contents.as_array() {
                    ...
                    page_dict.set("Contents", Object::Array(vec![Object::Reference(id), Object::Reference(watermark_id)]));
                } else {
                    page_dict.set("Contents", Object::Array(vec![Object::Reference(watermark_id)]));
                }
            } else {
