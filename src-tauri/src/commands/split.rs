        ...
        "ranges" => {
            ...
            for part in parts {
                ...
                if part.contains('-') {
                    ...
                    let start = bounds[0].trim().parse::<u32>().map_err(|_| format!("Invalid range '{}'", part))?;
                    let end = bounds[1].trim().parse::<u32>().map_err(|_| format!("Invalid range '{}'", part))?;
                    ...
                    let p = part.parse::<u32>().map_err(|_| format!("Invalid range '{}'", part))?;
                    if p < 1 || p > total_pages {
                        return Err(format!("Invalid range '{}': Out of bounds", part));
                    }
                    current_chunk.push(p);
