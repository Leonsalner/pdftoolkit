...
import { Loader2, FileJson, FileType } from 'lucide-react';
...
import { extractFormData } from '../lib/invoke';
...
export function FormDataPage({ notify, isActive }: FormDataPageProps) {
  const { t } = useI18n();
  const [filePath, setFilePath] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [outputName, setOutputName] = useState<string>('');
  const { recentFiles, addRecentFile } = useRecentFiles('forms');

  const { execute: performExtract, result, error, loading, reset } = useTauriCommand(extractFormData);
  ...
  const handleExtract = async () => {
    ...
    const res = await performExtract(filePath, outputName || undefined);
    if (res) {
      if (!isActive) {
        notify(t('forms.success'), 'forms');
      }
      if (fileName) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 animate-in fade-in slide-in-from-bottom-2 duration-400 ease-out h-full overflow-y-auto">
      <PageIntro page="forms" title={t('forms.title')} description={t('forms.desc')} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_2fr] gap-8 xl:gap-10">
        ...
        <div className={`space-y-5 transition-all duration-300 ${filePath ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 shadow-sm space-y-5">
            ...
            <div className="flex items-start gap-4 p-4 rounded-xl border border-[var(--cat-content)] bg-[var(--cat-content-bg)]">
              <div className="p-2 rounded-lg bg-[var(--cat-content)] text-white shrink-0">
                <FileJson size={18} />
              </div>
              <p className="text-xs text-[var(--text-primary)] leading-relaxed">
                This tool extracts all interactive form data (AcroForms) and saves it as a structured <strong>JSON</strong> file. Useful for data processing and automation.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="relative group">
                <input
                  ...
                  placeholder={fileName ? fileName.replace(/\.pdf$/i, '.json') : ''}
                ...
                />
                <FileType size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-disabled)] group-focus-within:text-[var(--text-secondary)] transition-colors" />
              </div>
            </div>
          </div>
          ...
          {error && <div className="animate-in fade-in zoom-in-95 duration-300"><ResultBanner type="error" message="Extraction failed" details={error} /></div>}
          {result && (
            <div className="animate-in fade-in zoom-in-95 duration-300">
              <ResultBanner
                type="success"
                message="Form Data Extracted"
                details={`Successfully extracted ${result.field_count} fields to ${result.output_path}`}
              />
            </div>
          )}
        </div>
